...
var welcomeTextIterations: number = 0;

function crypherText(startingText: string, endingText: string, element: HTMLElement, duration: number = 200) {
...
function getWelcomeText(): string {
    ...
    const maxTextIndex: number = welcomeTexts.length - 1
...
if (titleElement) {
    ...
    titleElement.addEventListener("mouseover", () => {
        crypherText(
            startingText,
            getWelcomeText(), // Random ending text
            titleElement
        );
    });
    ...
    titleElement.addEventListener("mouseout", () => {
        crypherText(
            titleElement.innerHTML, // Current randomized text
